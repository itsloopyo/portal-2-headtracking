...
:: Source Engine loads tier0.dll from <game>\bin with an altered search path,
:: so the winmm.dll proxy and the .asi must live in <game>\bin, NOT next to
:: portal2.exe - a root copy is never loaded. ASI_SUBDIR shifts the deploy
...
:: Source-engine deviation: deploy into the bin\ subdir (see CONFIG BLOCK).
