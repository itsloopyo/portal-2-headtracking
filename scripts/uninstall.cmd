...
:: Source Engine: the proxy and the .asi live in <game>\bin (see install.cmd),
:: so DEPLOY_DIR shifts into bin\ the same way.
set "ASI_SUBDIR=bin"
...
:: -------- Remove config + log the mod left at the game root --------
if defined ROOT_EXTRAS (
...
if /i "%FRAMEWORK_TYPE%"=="ASILoader" (
    ...
    rem Source-engine deviation: the payload lives in bin\ (see CONFIG BLOCK).
    if defined ASI_SUBDIR set "DEPLOY_DIR=!DEPLOY_DIR!\!ASI_SUBDIR!"
    exit /b 0
)
if /i "%FRAMEWORK_TYPE%"=="None" (
    for %%i in ("%GAME_PATH%\%GAME_EXE_RELPATH%") do set "DEPLOY_DIR=%%~dpi"
...
:: Source-engine deviation: the proxy lives in bin\ (see CONFIG BLOCK).
