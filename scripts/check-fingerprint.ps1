...
<#
Prints the PE fingerprint of the installed Portal 2 client.dll as a
...
rederive after a game patch. The mod routes on client.dll - not portal2.exe -
...
param(
    [string]$GamePath,
    [string]$DllRelPath = 'portal2\bin\client.dll'
)
...
if (-not $GamePath) {
    $GamePath = Find-GamePath -GameId 'portal-2'
    if (-not $GamePath) { throw "Portal 2 not found. Pass -GamePath explicitly." }
}
...
Write-Host "    { 0x0u, kViewSetupLayout_2025 },  // RenderView RVA - rederive, see .lab/NOTES.md"
Write-Host "};"
Write-Host ""
